...
// File: MERN-ExpenseManager/Frontend/expense-tracker/src/pages/Dashboard/Income.jsx
...
import DeleteAlert from '../../components/DeleteAlert';
import { API_PATHS } from '../../utils/apiPaths';
import { toast } from 'react-toastify'; // Assuming you have react-toastify for toasts
...
const Income = () => {
  ...
  const handleAddIncome = async (income) => {
    ...
    // CORRECTED LINE: Changed !isNaN(amount) to isNaN(amount)
    if (!amount || isNaN(amount) || Number(amount) <= 0) {
      toast.error('Amount should be a valid number greater than 0.');
      return;
    };
    ...
    try {
      ...
      toast.error('Failed to add income. Please try again.'); // User-friendly error message
    }
    ...
    <DashboardLayout activeMenu={"Income"}>
      <div className="my-5 mx-auto">
        ...
        >
          <DeleteAlert
            content="Are you sure you want to delete this income?"
            onDelete={() => DeleteIncome(openDeleteAlert.data)} // Corrected function call
          />
        </Modal>
      </div>
    </DashboardLayout>
